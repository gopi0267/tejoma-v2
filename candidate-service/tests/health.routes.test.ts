 ...
 * tenant-directory-service's tests/health.routes.test.ts exactly (this service owns a real
 * database, so /health and /ready are DB-backed, unlike jd-parser-service/api-gateway).
 */
...
describe('GET /health', () => {
  it('reports service identity and a timestamp', async () => {
    ...
    expect(res.body.service).toBe('candidate-service');
