/**
 * A real, minimal stand-in for the monolith's /internal/candidate-core/* API - mirrors
 * matching-scoring-service's/matching-evaluation-service's tests/helpers/mockMonolith.ts approach
 * (real HTTP on a real ephemeral port, not a mock of `fetch`), so the proxy route tests exercise
 * monolithClient.ts's actual network code. Per-path-prefix responses since this service's write
 * proxy endpoints need different shapes.
 */
