/**
 * Candidate Core Service Express app - Tier 0. Mirrors matching-evaluation-service's middleware
 * ordering and security defaults - this service now has a real, gateway-routed, staff-facing HTTP
 * surface (/api/candidates*, see routes/candidates.routes.ts, Step 3a of the remaining-monolith
 * migration) alongside its original internal-only /internal/* surface, so it needs real CORS/
 * cookie handling, not the internal-only `cors({ origin: false })` default.
 *
