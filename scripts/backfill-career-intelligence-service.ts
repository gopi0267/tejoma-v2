// One-time backfill: copies the monolith's role_profiles table into Career Intelligence Service's
// own database (tejoma_career_intelligence). role_profiles ONLY - career_trajectories is NOT
// backfilled, same reasoning as Batches 26/27's owned tables: this service computes its own
// independent copy of career_trajectories, backfilling the monolith's version would conflict with
// (not seed) that independent computation. See backfill-identity-service.ts's header comment for
// the full methodology (read-only against the monolith, upsert-by-id, safe to re-run).
//
// Usage:
//   npx tsx scripts/backfill-career-intelligence-service.ts             (writes for real)
//   npx tsx scripts/backfill-career-intelligence-service.ts --dry-run    (reports counts only)
...
async function main() {
  ...
  console.log(`=== Backfilling Career Intelligence Service (${TARGET_DB_NAME}) ${dryRun ? '[DRY RUN]' : ''} ===\n`);
