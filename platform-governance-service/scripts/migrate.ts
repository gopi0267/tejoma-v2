/**
 * Migration runner for Platform Governance Service's database. Mirrors
 * identity-service/scripts/migrate.ts exactly (same tracked schema_migrations approach, same
 * usage) - see that file's header comment for the full reasoning.
 *
 * Usage:
  ...
  if (!confirmed) {
    console.error(
      'Refusing to run a rollback migration without --confirm. This is a destructive operation ' +
        'that may delete real data once this database holds real registration requests - see ' +
