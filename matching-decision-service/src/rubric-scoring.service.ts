 ...
 * independent of the platform's real matching engine (matching-scoring-service's own
 * calculateMatchScore, reached via matchingScoringServiceClient - which drives the swipe queue's
 * match % and "AI Match Breakdown") - nothing here feeds back into that pipeline, and nothing
 * there is modified by this file.
 *
 ...
 *
 * Ported verbatim from the monolith's own src/rubric-scoring.service.ts (write-cutover completion
 * plan, Phase D) - only the client-construction pattern changed, to match this service's own
 * (and chat-service's/matching-skill-discovery-service's) eager-construction convention: built
 * once at module scope from config/env.ts's GEMINI_API_KEY, which is startup-validated
 * (REQUIRED_ALWAYS) rather than lazily checked on first use like the monolith's own getClient().
 */
import { GoogleGenAI, Type } from '@google/genai';
import { GEMINI_API_KEY } from './config/env.js';
import { Job, Candidate } from './types.js';

// Same model as the monolith's own rubric-scoring.service.ts/chat.routes.ts - the only one
// confirmed reachable on the available API key/quota (gemini-2.5-flash/pro, gemini-2.0-flash*,
// gemini-3.x/pro-latest all fail).
