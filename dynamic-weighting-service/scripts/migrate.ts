/**
 * Migration runner for Dynamic Weighting / Explainable Matching Service's database. Mirrors every
 * other Tier 0 service's scripts/migrate.ts exactly.
 *
 * Usage:
  ...
  if (!confirmed) {
    console.error(
      'Refusing to run a rollback migration without --confirm. This is a destructive operation ' +
        'that may delete real data once this database holds real dual-written mirror data - see ' +
