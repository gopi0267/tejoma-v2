/**
 * Dynamic Weighting / Explainable Matching Service Express app - Tier 0 (Batch 33). Mirrors every
 * other Tier 0 service's middleware ordering and security defaults exactly, minus auth
 * middleware - this service has no user-facing HTTP surface (see config/env.ts's header comment).
 *
...
app.use(cors({ origin: false })); // no browser ever calls this service - internal, network-boundary-trusted only

app.use(express.json({ limit: '5mb' })); // hybrid-retrieve can carry many candidates' feature data
