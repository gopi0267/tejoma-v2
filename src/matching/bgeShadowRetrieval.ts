// BGE-M3 + BGE-Reranker-v2-m3 retrieval shadow comparison. SHADOW MODE ONLY - never affects
// which candidates a recruiter sees or their order. Compares the existing live ranking (BM25 +
// MiniLM hybrid retrieval / dynamic weighting, entirely unchanged) against what a BGE-M3 dense
// retrieval + BGE-Reranker-v2-m3 cross-encoder pass over the SAME pool would have produced.
//
// WHY SHADOW-ONLY, WITH REAL NUMBERS: benchmarked on this project's own CPU dev machine (no GPU
// exists here) before writing any integration code:
//   - Single BGE-M3 embedding: ~3,318ms (vs. the existing MiniLM model's "well under 100ms")
//   - Reranking 100 pairs with BGE-Reranker-v2-m3: ~58,100ms total (581ms/pair) - about 580x
//     slower than a GPU-accelerated deployment's expected ~1ms/pair
// That gap is why this is a genuinely separate, opt-in-only capability, not a replacement for
// the live retrieval path - see bgeRetrievalClient.ts's module doc. The same code is meant to run
// unchanged against a real GPU deployment later (sentence-transformers auto-detects CUDA), at
// which point these numbers would look very different - but nothing here assumes that yet.
//
...
import { embedWithBge, embedBatchWithBge, rerankWithBge, isBgeServiceAvailable } from './bgeRetrievalClient.js';
import type { Candidate, Job, RankingEntry } from '../types.js';
...
export async function computeBgeShadowComparison(job: Job, existingRanked: Array<{ candidate: Candidate; match_score: number }>): Promise<BgeShadowComparisonResult> {
...
// Fire-and-forget, same discipline as every other shadow signal in this codebase - never blocks
...
export function logBgeShadowComparisonInBackground(companyId: number, job: Job, existingRanked: Array<{ candidate: Candidate; match_score: number }>): void {
  computeBgeShadowComparison(job, existingRanked)
    .then((result) =>
      db.insertBgeRetrievalShadowComparison({
    ...
    .catch((err) => console.error('BGE shadow retrieval comparison failed:', err?.message ?? err));
}
