// Enterprise AI Matching Architecture, §2.4 Career Intelligence Platform - Module 3: Stability &
// Domain Analysis.
//
// FAIRNESS-CRITICAL MODULE, PER THE ARCHITECTURE DOC ITSELF: "Career stability and tenure-pattern
// signals are a plausible vector for indirect discrimination - tenure gaps correlate with
// parental leave, caregiving, disability, and other protected circumstances having nothing to do
// with candidate quality. This signal must never be used as a silent penalty; if used at all, it
// requires an explicit governance policy, not just an engineering feature."
//
// This module therefore reports FACTS ONLY:
//   - A gap's start date, end date, and duration in months - never a cause.
//   - A tenure pattern label (stable/short/variable) - a description of what's observed, never an
//     interpretation ("unstable", "job-hopper", etc.) and never a score.
//   - NOT wired into any live scoring or screening path this phase, or any future phase without
//     the explicit governance approval the architecture doc itself requires.
...
const MIN_GAP_MONTHS = 2; // below this, treat as normal rounding/reporting noise, not a real gap
...
// Facts only - see module doc. A gap is only recorded when BOTH the preceding role's end and the
// following role's start are real, parseable dates; an undated boundary is never guessed into a
// gap (or into "no gap").
