// Enterprise AI Matching Architecture, Phase 3 - Evaluation Framework.
//
...
// see migration-recruiter-review.sql/swipe.routes.ts's recordSwipe) + swipes.action (0/0.5/1,
// graded ground-truth relevance) grouped by job_id - not a synthetic or assumed dataset. This is
// this database's actual swipe history, so results are only as robust as that history is deep;
// see evaluateFromSwipes' data-volume note below, which is reported honestly rather than masked.
//
// "Relevant" (for Precision@K/Recall@K/MRR, which need a binary cutoff) is action > 0 - a save
// (0.5) counts as relevant alongside a full accept (1), same "a save leans positive" reasoning
// already used by feedbackSignals.ts. NDCG@K uses the full graded relevance (0/0.5/1) directly,
// which is where the distinction between a save and a full accept actually matters.
...
// Each takes a single query's relevance grades, already ordered by the ranking being evaluated
// (rank 1 first).
...
export interface QueryEvaluation {
  jobId: number;
  rankedRelevances: number[]; // ordered by the ranking being evaluated
}
...
// Macro-average across jobs (each job/query weighted equally, not each swipe) - the standard
// convention for these metrics, and the right one here: a job with many swipes shouldn't drown
// out the ranking quality signal from jobs with fewer.
...
export async function evaluateFromSwipes(companyId: number, k: number = DEFAULT_K): Promise<EvaluationResult> {
  ...
  for (const swipe of swipes) {
    if (typeof swipe.match_score !== 'number') continue; // no predicted rank basis - skip, never guess
  ...
  for (const [jobId, jobSwipes] of byJob) {
    if (jobSwipes.length < 2) continue; // a single-candidate "ranking" has no order to evaluate
