...
// requireCandidateAuth is applied per-route below (not via a blanket router.use()) - this
// router is mounted at the shared '/api' prefix alongside routers for completely different,
// staff-only paths (jobs, candidates, swipes, ...) that are reached later in the middleware
// chain. A path-unscoped router.use(requireCandidateAuth) here would intercept every request
// that reaches this router looking for a candidate cookie - including staff requests headed
// for those later routers - and 401 them before they ever got there. Each route below gates
// only its own path, exactly like candidate-auth.routes.ts already does for /candidate-auth/me.
const PROFILE_FIELDS = ['headline', 'skills', 'years_of_experience', 'location', 'education', 'summary'] as const;

// Onboarding-wizard additions to the completion metric (approved explicitly for this feature -
// unlike Phase 1's original fixed-7 decision, the candidate now builds a genuinely richer
// profile through the wizard, so the % should reflect that).
const PROFILE_FIELDS_V2 = ['course_name', 'primary_skill'] as const;

/**
 * Completion is measured across name + the 6 original profile fields + the 2 onboarding-wizard
 * fields above + one "experience section complete" signal (a saved candidate_experiences row,
 * or years_of_experience === 'Fresher' for candidates who have none) - passed in separately
 * since it requires a query against a different table.
 */
...
// ==================== UPDATE OWN PROFILE ====================
router.put('/candidate-profile/me', requireCandidateAuth, async (req, res) => {
  try {
    const {
...
router.get('/candidate-profile/experiences', requireCandidateAuth, async (req, res) => {
  try {
    const experiences = await db.getCandidateExperiences(req.candidate!.candidate_id);
...
router.post('/candidate-profile/experiences', requireCandidateAuth, async (req, res) => {
  try {
...
router.put('/candidate-profile/experiences/:id', requireCandidateAuth, async (req, res) => {
  try {
...
router.delete('/candidate-profile/experiences/:id', requireCandidateAuth, async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10);
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'Invalid experience id' });
