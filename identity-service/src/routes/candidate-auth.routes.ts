/**
 * Candidate authentication routes - ported from the monolith's src/api/candidate-auth.routes.ts.
 * Batch 5 covered login, refresh, logout, logout-all, and me. This batch (8) adds registration
 * (OTP-based), forgot-password (OTP-based), and Google OAuth (auto-provisioning, matching the
 * monolith's candidate behavior exactly - unlike staff, which does not auto-provision).
 *
 * IMPORTANT SCOPE NOTE, carried from Batch 5: the monolith's `toCandidateInfo()` returns profile
 * fields (headline, skills, years_of_experience, location, education, summary,
 * onboarding_completed_at) that Identity DB's candidate_accounts table does NOT have (Phase
 * 3(database) section 4's auth/profile split, executed in Batch 3). This service returns only the
 * fields it actually owns (id, name, email, phone) plus a `profile` object resolved via
 * services/marketplaceClient.ts, which is null until Marketplace Service exists (Tier 1, out of
 * this document's scope). See that client's header comment for the full issue/reason/impact/fix
 * writeup. Practical consequence: real candidate-auth traffic cutover is gated on Tier 1's
 * Marketplace Service, not just this service being ready - tracked explicitly as a Tier 0 exit
 * caveat.
 *
 * IMPLEMENTATION ISSUE, this batch, resolved per the required methodology (NOT implemented here -
 * documented as Blocked instead of faked):
 *   Problem: the monolith has `PUT /candidate-auth/complete-onboarding`, which sets
 *     candidate_accounts.onboarding_completed_at directly.
 *   Why it exists: in the monolith's single table, onboarding completion is just another column
 *     on the same row as the auth columns.
 *   Impact: Identity DB's candidate_accounts (Batch 3) is the auth-column slice only -
 *     onboarding_completed_at is a PROFILE column, owned by Marketplace Service's database (Tier
 *     1, out of this document's Tier 0 scope). Identity Service has no table to write this flag
 *     into, and marketplaceClient.ts today only exposes a read (getCandidateProfile) - adding a
 *     write there would call an endpoint on a service that does not exist yet and cannot, by
 *     definition, durably persist anything. Returning a fake 200 here would violate the "never
 *     falsely mark anything as implemented" rule directly - the flag would appear to be set from
 *     the client's perspective while nothing durable actually happened anywhere.
 *   Minimum change: NOT implemented in this batch. Explicitly BLOCKED on Marketplace Service
 *     (Tier 1). Tracked the same way Batch 4's staff `signup/start` is blocked on Tenant Directory
 *     Service (Tier 0, later batch) - not a new kind of gap, the same kind, applied here.
 */
...
import { getCandidateProfile } from '../services/marketplaceClient.js';
...
import type { CandidateAccount, OtpPurpose } from '../types.js';

const router = Router();

// Modeled directly on auth.routes.ts's equivalent constants - kept as separate copies (rather
// than importing from auth.routes.ts) so the candidate auth surface has zero coupling to the
// staff auth route module, exactly matching the monolith's own separation.
const OTP_TTL_MS = 10 * 60 * 1000; // 10 minutes
const OTP_RESEND_COOLDOWN_MS = 60 * 1000;
const OTP_MAX_REQUESTS_PER_HOUR = 5;
const OTP_PURPOSE: OtpPurpose = 'candidate_signup';
const FORGOT_PASSWORD_OTP_PURPOSE: OtpPurpose = 'candidate_reset';

// Candidate Google OAuth is optional - if unset, /google responds with a clear redirect instead
// of crashing, and the client-side "Continue with Google" button always renders. Unlike staff
// Google OAuth (Batch 7), candidate sign-in DOES auto-provision an account on first Google
// sign-in - this mirrors the monolith's actual, deliberate behavior difference between the two
// flows exactly (see this file's callback handler below), not a new decision.
const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID || '';
const GOOGLE_CLIENT_SECRET = process.env.GOOGLE_CLIENT_SECRET || '';
const GOOGLE_REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI || '';
const candidateGoogleOAuthConfigured = Boolean(GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET && GOOGLE_REDIRECT_URI);
const candidateGoogleClient = candidateGoogleOAuthConfigured
  ? new OAuth2Client(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI)
  : null;
if (!candidateGoogleOAuthConfigured) {
...
async function toCandidateInfo(candidate: CandidateAccount) {
  ...
  return {
    ...
    // Profile fields deferred to Marketplace Service (see header note) - null until that service
    // exists and is wired up, exactly mirroring auth.routes.ts's toCompanyInfo deferral pattern.
...
async function findCandidateByIdentifier(identifier: { type: 'email' | 'phone'; value: string }): Promise<CandidateAccount | null> {
...
/** Mirrors auth.routes.ts's auditContext - see migrations/002_audit_log.up.sql. */
...
async function enforceOtpRequestLimits(identifier: { type: 'email' | 'phone'; value: string }, purpose: OtpPurpose = OTP_PURPOSE): Promise<string | null> {
...
/** Issues a candidate access+refresh token pair, persists the refresh token, and sets cookies. Mirrors auth.routes.ts's issueSession. */
async function issueCandidateSession(req: Request, res: Response, candidate: CandidateAccount, remember: boolean = true): Promise<string> {
...
router.post('/candidate-auth/register/complete', async (req, res) => {
  try {
    ...
    await db.recordAuditEvent({ actorType: 'candidate', actorId: candidate.id, eventType: 'registration', ...auditContext(req) });
    res.status(201).json({ access_token, candidate: await toCandidateInfo(candidate) });
  } catch (error) {
...
router.post('/candidate-auth/login', async (req, res) => {
  try {
    ...
    const identifier = normalizeIdentifier(rawIdentifier);
    if (!identifier) {
      await db.recordAuditEvent({ actorType: 'candidate', actorId: null, eventType: 'login_failed', ...auditContext(req), metadata: { reason: 'invalid_identifier_format' } });
      return res.status(401).json({ error: 'Invalid email or password' });
    }

    const candidate = await findCandidateByIdentifier(identifier);
    if (!candidate) {
      await db.recordAuditEvent({ actorType: 'candidate', actorId: null, eventType: 'login_failed', ...auditContext(req), metadata: { reason: 'unknown_identifier', identifier: identifier.value } });
      return res.status(401).json({ error: 'Invalid email or password' });
    }
    if (!candidate.is_active || candidate.deleted_at) {
      await db.recordAuditEvent({ actorType: 'candidate', actorId: candidate.id, eventType: 'login_failed', ...auditContext(req), metadata: { reason: 'account_deactivated' } });
      return res.status(401).json({ error: 'This account has been deactivated' });
    }

    const passwordMatch = await bcrypt.compare(password, candidate.password_hash);
    if (!passwordMatch) {
      await db.recordAuditEvent({ actorType: 'candidate', actorId: candidate.id, eventType: 'login_failed', ...auditContext(req), metadata: { reason: 'wrong_password' } });
      return res.status(401).json({ error: 'Invalid email or password' });
    }

    const access_token = await issueCandidateSession(req, res, candidate, remember !== false);
    await db.recordAuditEvent({ actorType: 'candidate', actorId: candidate.id, eventType: 'login_success', ...auditContext(req) });
    res.json({ access_token, candidate: await toCandidateInfo(candidate) });
  } catch (error) {
...
router.post('/candidate-auth/refresh', async (req, res) => {
  try {
    ...
    if (record.revoked_at) {
      ...
      await db.recordAuditEvent({ actorType: 'candidate', actorId: record.candidate_id, eventType: 'refresh_token_reuse_detected', ...auditContext(req) });
      clearCandidateAuthCookies(res);
...
router.get('/candidate-auth/me', requireCandidateAuth, async (req, res) => {
  try {
    ...
    res.json({ candidate: await toCandidateInfo(candidate) });
  } catch (error) {
...
router.post('/candidate-auth/logout', async (req, res) => {
  try {
    ...
    if (rawToken) {
      ...
      if (record) {
        await db.recordAuditEvent({ actorType: 'candidate', actorId: record.candidate_id, eventType: 'logout', ...auditContext(req) });
      }
    }
  } catch (error) {
...
router.post('/candidate-auth/logout-all', async (req, res) => {
  try {
    ...
    if (rawToken) {
      ...
      if (record) {
        await db.revokeAllCandidateRefreshTokensForCandidate(record.candidate_id);
        await db.recordAuditEvent({ actorType: 'candidate', actorId: record.candidate_id, eventType: 'logout_all', ...auditContext(req) });
      }
    }
  } catch (error) {
...
router.post('/candidate-auth/forgot-password/reset', async (req, res) => {
  try {
    ...
    // Ported exactly from the monolith - not a gap introduced by this migration.
    ...
    await db.recordAuditEvent({ actorType: 'candidate', actorId: candidate.id, eventType: 'password_reset', ...auditContext(req) });
...
router.get('/candidate-auth/google', (req, res) => {
  if (!candidateGoogleClient) {
    return res.redirect('/candidate?auth_error=google_not_configured');
  }
  const url = candidateGoogleClient.generateAuthUrl({
    access_type: 'online',
...
router.get('/candidate-auth/google/callback', async (req, res) => {
  ...
  try {
    ...
    const { tokens } = await candidateGoogleClient.getToken(code);
    if (!tokens.id_token) {
      return res.redirect('/candidate?auth_error=google_auth_failed');
    }

    // Identity comes only from Google's verified ID token - never from request body/query.
    const ticket = await candidateGoogleClient.verifyIdToken({ idToken: tokens.id_token, audience: GOOGLE_CLIENT_ID });
    ...
    if (!candidate) {
      ...
      // Auto-provisioning here (unlike staff Google OAuth in Batch 7, which requires an existing
      // account) is not a new decision - it mirrors the monolith's real, deliberate behavior
      // difference between candidate and staff sign-in.
    ...
    await db.recordAuditEvent({ actorType: 'candidate', actorId: candidate.id, eventType: isNewAccount ? 'oauth_signup' : 'oauth_signin', ...auditContext(req) });
...
// NOTE: PUT /candidate-auth/complete-onboarding is intentionally NOT implemented in this batch -
// see this file's header comment for the full issue/reason/impact writeup. It is BLOCKED on
// Marketplace Service (Tier 1), which owns the onboarding_completed_at column. Implementing a
// stub here that returns success without persisting anything durable would violate the "never
// falsely mark anything as implemented" rule.
