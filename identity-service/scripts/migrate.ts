/**
 * Migration runner for Identity Service's database. A real, tracked migration system (a
 * schema_migrations table recording what has been applied) - not the monolith's existing
 * run-migration.js, which runs one hardcoded SQL file against a hardcoded connection with no
 * tracking. This is deliberately more rigorous, since this batch was explicitly asked to produce
 * "production migrations... forward migrations, rollback migrations, validation scripts."
 *
 * Usage:
 ...
 *
 * Connection: reads DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD from the environment, the same
 * convention as src/db.ts. Never hardcodes a connection string or credential.
 */
import fs from 'fs';
...
async function ensureMigrationsTableExists(): Promise<void> {
  // The migration files themselves create schema_migrations (see 001_initial_schema.up.sql),
  // but the runner needs to be able to query it even before any migration has run - so it
  // creates it defensively here too, idempotently, matching the same table shape exactly.
  await pool.query(`
  ...
  if (!confirmed) {
    console.error(
      'Refusing to run a rollback migration without --confirm. This is a destructive operation ' +
        'that may delete real data once this database holds real user/session records - see ' +
