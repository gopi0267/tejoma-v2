 ...
 * route tests exercise the real client network code. Job Service starts THREE of these (one each
 * for the monolith, candidate-core-service, and matching-scoring-service - see
 * tests/jobs.routes.test.ts), since GET /api/jobs/:id's real cutover calls all three independently.
 * Per-path-prefix responses since different endpoints on the same mock need different shapes.
 */
