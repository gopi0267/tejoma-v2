...
async function queryGemini(systemInstruction: string, promptText: string, options: GeminiCallOptions = {}): Promise<any> {
  ...
  for (let attempt = 1; attempt <= MAX_GEMINI_RETRIES; attempt++) {
    ...
    // Combine the caller's cancellation signal (browser disconnect) with our own timeout,
    // so either one actually stops the in-flight call instead of leaving it orphaned.
...
export async function parseResume(resumeText: string, options?: { maxRetries?: number; fallbackToRegex?: boolean; signal?: AbortSignal }) {
  ...
  try {
    ...
    // Pass 1 missed. The user explicitly asked for maximum accuracy over speed and confirmed
    // extra tool calls are fine, so we always run this second pass rather than treating it as
    // optional - unlike the old local-Ollama pipeline, a second cloud call costs seconds, not
    // minutes.
    ...
    try {
      ...
      // If the audit pass fails for any reason, fall back to the still-valid draft rather than
      // failing the whole resume - a single successful pass is far better than none.
    ...
    // Split on comma OR semicolon regardless of which separator the model actually used for a
    // given field - the prompt asks for commas on most list fields and semicolons specifically
    // for Previous_Companies/Certifications, but the model doesn't always honor that distinction
    // consistently, and a single wrong separator would otherwise collapse an entire list into
    // one giant blob string instead of individual items.
    ...
    // Post-extraction sanity check for Email - guard against the model paraphrasing/inventing one.
