/**
 * A real, minimal stand-in for the monolith's /internal/analytics/* API - mirrors
 * recruiting-service's tests/helpers/mockMonolith.ts approach (real HTTP on a real ephemeral
 * port, not a mock of `fetch`), so the proxy route tests exercise monolithClient.ts's actual
 * network code. Unlike single-endpoint services, this one needs per-path-prefix responses since
 * the four routes hit four different monolith paths.
 */
