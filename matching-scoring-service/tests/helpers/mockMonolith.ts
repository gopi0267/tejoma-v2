/**
 * A real, minimal stand-in for the monolith's /internal/matching-scoring/* API - mirrors
 * matching-evaluation-service's/chat-service's tests/helpers/mockMonolith.ts approach (real HTTP
 * on a real ephemeral port, not a mock of `fetch`), so the proxy route tests exercise
 * monolithClient.ts's actual network code. Per-path-prefix responses since this service's ML admin
