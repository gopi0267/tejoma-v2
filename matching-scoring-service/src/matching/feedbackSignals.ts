...
//
// Ported VERBATIM from the monolith's src/matching/feedbackSignals.ts as part of moving
// POST /api/ml/train off the monolith. Pure label/weight resolution - no database access, no
// HTTP, no monolith coupling - so it moves unchanged; only the `Swipe` type import path differs.
// Kept as its own module (not inlined into services.ts) exactly as the monolith had it, so the
// two copies stay diffable while the monolith remains as rollback.
//
...
// Minimal structural type - only the four fields resolveTrainingSamples actually reads. Declared
// locally rather than importing the monolith's full Swipe interface (which carries recruiter-review
// columns irrelevant here), so this module has no dependency outside its own service.
