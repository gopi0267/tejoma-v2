/**
 * Migration runner for Tenant Directory Service's database. Mirrors identity-service/
 * platform-governance-service's scripts/migrate.ts exactly.
 *
 * Usage:
  ...
  if (!confirmed) {
    console.error(
      'Refusing to run a rollback migration without --confirm. This is a destructive operation ' +
        'that may delete real data once this database holds real company records - see ' +
