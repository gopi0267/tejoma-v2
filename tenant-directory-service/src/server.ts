/**
 * Tenant Directory Service Express app - Tier 0 (Phase 11/12). Mirrors identity-service/
 * platform-governance-service's server.ts middleware ordering and security defaults exactly.
 *
 * This module defines and exports the Express `app` only - it never binds a port, for the exact
 * same reason identity-service/src/server.ts documents - index.ts is the sole process entry point.
 */
...
// Not mounted under /api - this service exposes no public API surface at all, only /internal/*
// (see config/env.ts's header comment on the network-boundary trust model).
